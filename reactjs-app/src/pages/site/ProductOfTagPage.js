...
const ProductOfTagPage = () => {
        ...
        const fetchProducts = async () => {
            try {
                ...
                if (Array.isArray(productTagResponse)) {
                    const productDetailsPromises = productTagResponse.map(productCategory =>
    ...
    const addProductToFavorite = async (productId) => {
        ...
        if (!user) {
            navigate("/login", { state: { redirectTo: `/product-of-tag/${id}` } });
            return;
        }
        else {
            const favoriteAdd = {
...
export default ProductOfTagPage;
